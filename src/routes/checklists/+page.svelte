<script lang="ts">
  ...
  const handleDelete = async (checklist: Record<string, any>) => {
    ...
    const speciesCount = await speciesCollection
      .where('checklistID')
      .equals(checklist.checklistID)
      .count();
    if (speciesCount > 0) {
      alert(
  ...
  const handleDeleteAll = async () => {
    alert(
      'Cannot delete all checklists. Please delete individually, and proceed with caution!',
    );
  };
</script>
