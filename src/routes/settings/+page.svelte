<script lang="ts">
  ...
  const handleProjectChange = async () => {
    if (settingsRecord.project) {
      projectSurveyCollection
        .where('projectID')
        .equals(settingsRecord.project!.projectID!)
        .toArray()
        .then((surveys) => {
  ...
  const validateSettings = () => {
    if (
      !settingsRecord.user?.firstName ||
      !settingsRecord.user?.lastName ||
      !settingsRecord.user?.userInitials
    ) {
...
<main class="p-4 flex flex-col gap-4">
  ...
  <div class="flex justify-end gap-4">
    <button type="button" class="btn btn-primary" onclick={handleSaveSettings}
