...
export const load: PageLoad = async () => {
  ...
  if (settings && settings.project) {
    const dbProject = projects.find(
      (p) => p.projectID === settings.project!.projectID,
    );
    if (dbProject) {
      // it should always be true unless one deletes the project
      settings.project = dbProject;
    } else {
      alert(
        'The project you previously selected is missing from the database!',
      );
    ...
    if (settings.project) {
      ...
      if (settings.projectSurvey) {
        const dbProjectSurvey = projectSurveys.find(
          (ps) => ps.surveyID === settings.projectSurvey!.surveyID,
        );
        if (dbProjectSurvey) {
          // it should always be true unless it was deleted
          settings.projectSurvey = dbProjectSurvey;
        } else {
          alert(
            'The project survey you previously selected is missing from the database!',
          );
  ...
  if (settings && settings.checklist) {
    const dbChecklist = checklists.find(
      (c) => c.checklistID === settings.checklist!.checklistID,
    );
    if (dbChecklist) {
      ...
      alert(
        "The checklist you previously selected is missing from the database or doesn't have a species count!",
      );
