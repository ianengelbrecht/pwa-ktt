...
export const load: PageLoad = async ({ url }) => {
  ...
  if (!checklistID) {
    ...
    error(400, 'Oops, we should not have got here without a checklistID for the species list');
  }
  ...
  try {
    ...
    if (!checklist) {
      //redirect to error page
      error(500, `Whoah! The checklist with ID ${checklistID} does not exist in the database!`);
    }
  }
  catch (e) {
  ...
  try {
    species = await speciesCollection.where('checklistID').equals(checklistID!).toArray()
  }
  catch (e) {
    if (e instanceof Error) {
