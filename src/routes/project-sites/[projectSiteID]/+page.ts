...
export const load: PageLoad = async ({ params, url }) => {
  ...
  if (!projectSiteID) {
    error(400, {
      message:
        'Oops, you got here with no projectSiteID! That should not have happened...',
    });
  }
  ...
  if (projectSiteID != 'new') {
    ...
    if (!projectSite) {
      error(
        500,
        'Whoah! There is no projectSite with ID ' +
          projectSiteID +
  ...
  const index = otherProjectSites.findIndex(
    (site) => site.projectSiteID === projectSiteID,
  );
  if (index > -1) {
