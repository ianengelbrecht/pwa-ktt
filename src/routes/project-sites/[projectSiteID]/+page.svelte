<script lang="ts">
  ...
  const siteCodeValidation = () => {
    ...
    if (
      otherProjectSites.some(
        (site) =>
          site.siteCode?.trim().toLowerCase().replace(/\s+/g, '') ===
          projectSiteRecord.siteCode?.trim().toLowerCase().replace(/\s+/g, ''),
      )
    ) {
      toast.push(`Site code ${projectSiteRecord.siteCode} already exists`, {
  ...
  const validateProjectSiteRecord = () => {
    const missingFields = [];
    if (
      !projectSiteRecord.siteCode ||
      projectSiteRecord.siteCode.trim() === ''
    ) {
