<script lang="ts">
  ...
  let showSearch = $state(
    localStorage.getItem(page.url.pathname + 'showSearch') === 'true',
  );
  let searchTerm: string = $state('');

  let showSort = $state(
    localStorage.getItem(page.url.pathname + 'showSort') === 'true',
  );
  let sortField: { index: number; value: string; label: string } | null =
    $state(null);
  ...
  const handleNewClick = () => {
    const newURL =
      page.url.pathname + '/new' + (page.url.search ? page.url.search : ''); // searchString is '' by default
    goto(newURL);
  };
</script>
...
{#if items.length === 0}
  <div class="text-center text-slate-500 text-lg font-semibold">
    No {page.url.pathname.replace(/-/g, ' ').replace(/\//g, '')} recorded
  </div>
{:else}
  <div class="flex justify-end gap-4 text-slate-500">
    {#if sortable}
      <button class="cursor-pointer" onclick={handleSortClick}>
        <span
          class={['material-symbols-outlined', { 'text-slate-200 ': showSort }]}
          style="font-size: 2.5rem;">sort</span
        >
      </button>
    {/if}
    <button class="cursor-pointer" onclick={handleSearchClick}>
      <span
        ...
        style="font-size: 2rem;">search</span
      >
    </button>
  </div>
  ...
  <div class="w-full flex flex-wrap items-center gap-2 mb-4">
    {#if showSearch}
      <input
        type="text"
        class="w-full md:w-1/2 lg:w-1/3 shrink-0 h-10 p-2 rounded bg-white text-black"
        placeholder="Search"
    ...
    {#if showSort && sortable}
      <div
        class="w-full md:w-1/2 lg:w-1/3 shrink-0 ml-auto flex items-center gap-2"
      >
        <Select
        ...
        >
          <span
            class="material-symbols-outlined"
            class:flip-icon={ascending}
            style="font-size: 2.5rem; transition: transform 0.2s ease-in-out;"
            >sort</span
          >
        </button>
      </div>
  ...
  <div class="flex justify-left gap-8">
    {#if exportFunction}
      <button
        onclick={() => exportFunction(displayData)}
        class="mt-4 p-2 bg-green-600 rounded cursor-pointer disabled:bg-transparent disabled:cursor-auto"
        disabled={!items.length}>Export CSV</button
      >
    {/if}
    {#if deleteAll}
      <button
        onclick={confirmAndDeleteAll}
        class="mt-4 p-2 rounded border-2 border-gray-200 cursor-pointer disabled:cursor-auto"
        disabled={!items.length}>Delete all</button
      >
    {/if}
  </div>
{/if}
...
<button
  class="fixed right-4 bottom-4 text-white bg-green-400 rounded-full shadow-xl cursor-pointer w-15 h-15 flex items-center justify-center border-4 border-slate-800 hover:outline-2 hover:outline-white"
  onclick={handleNewClick}
>
  <span class="material-symbols-outlined leading-none" style="font-size: 3rem;"
    >add</span
  >
</button>
...
<dialog
  ...
  class="rounded-xl p-6 shadow-lg w-80 m-auto"
  bind:this={dialog}
>
  <p class="text-lg font-semibold mb-4">{dialogMessage}</p>
  <form method="dialog">
    <menu class="flex justify-end gap-3">
      <button value="cancel" class="px-4 py-2 rounded bg-gray-200"
        >Cancel</button
      >
      <button
        id="confirmButton"
        value="confirm"
        class="px-4 py-2 rounded bg-red-600 text-white">Confirm</button
      >
    </menu>
  </form>
</dialog>
